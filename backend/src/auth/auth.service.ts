...
export class AuthService {
  ...
  async register({ email, password, first_name, last_name }: RegisterDto): Promise<ResponseDto> {
    ...
    return { message: 'User created succesfully', email: userData.email, token: token }
  }
  ...
  async login({ email, password }: LoginDto): Promise<ResponseDto> {
    ...
    return { message: 'user successfully logged in', email: user.email, token: token }
  }
}
